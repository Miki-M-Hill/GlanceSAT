#!/usr/bin/env python3
"""
Rubric v1.1 - batch_002: rows 251–500 of Database.json (words after batch_001).

`memoryHook` values come from **scripts/batch_002_user_memory_hooks.json**
(user-curated). Source TSV: **scripts/batch_002_hooks.tsv** (run emit_batch_002_hooks_json.py after edits).

Output: GlanceSAT/GlanceSAT/patches_batch_002.json with:
  - `entries`: 250 FULL word objects (merge into Database.json by `id`)
  - `batch_stats`, `flags`
"""
...
OUT = ROOT / "GlanceSAT" / "GlanceSAT" / "patches_batch_002.json"
USER_HOOKS_JSON = ROOT / "scripts" / "batch_002_user_memory_hooks.json"
...
NEGATIVE = {
    ...
    "complicit",
    # batch_002 (partial; extend as you tune the corpus)
...
def memory_hooks_for_batch(batch: list[dict], user: dict[str, dict | None], flags: list[dict]) -> dict[str, dict | None]:
    ...
    for row in batch:
        ...
        if w not in user:
            flags.append(
                {"id": row.get("id"), "word": w, "reason": "Word missing from batch_002_user_memory_hooks.json"}
            )
            out[w] = None
        else:
            hook = user[w]
...
def main() -> None:
    with DB.open(encoding="utf-8") as f:
        data = json.load(f)
    batch = data[BATCH_OFFSET : BATCH_OFFSET + BATCH_SIZE]
    if len(batch) != BATCH_SIZE:
        raise SystemExit(f"Need at least {BATCH_OFFSET + BATCH_SIZE} rows in Database.json")
