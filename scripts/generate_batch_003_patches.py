#!/usr/bin/env python3
"""
Rubric v1.1 - batch_003: rows 501–750 of Database.json (words after batch_002).

`memoryHook` values come from **scripts/batch_003_user_memory_hooks.json**
(user-curated). Source TSV: **scripts/batch_003_hooks.tsv** (run emit_batch_003_hooks_json.py after edits).

Output: GlanceSAT/GlanceSAT/patches_batch_003.json with:
...
OUT = ROOT / "GlanceSAT" / "GlanceSAT" / "patches_batch_003.json"
USER_HOOKS_JSON = ROOT / "scripts" / "batch_003_user_memory_hooks.json"
...
def memory_hooks_for_batch(batch: list[dict], user: dict[str, dict | None], flags: list[dict]) -> dict[str, dict | None]:
    ...
    for row in batch:
        ...
        if w not in user:
            flags.append(
                {"id": row.get("id"), "word": w, "reason": "Word missing from batch_003_user_memory_hooks.json"}
            )
            out[w] = None
        else:
            hook = user[w]
